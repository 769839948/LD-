...
- (void)_loadMessagesDirection:(BOOL)direction
{
    ...
    dispatch_async(_searchMessageQueue, ^{
        ...
        if (direction) {
            moreMessages = [self.conversation loadMoreMessagesFromId:self.upMessageId limit:SEARCHCHAT_PAGE_SIZE];
            ...
            moreMessages = [self.conversation loadMoreMessagesFromId:self.downMessageId limit:SEARCHCHAT_PAGE_SIZE];
            if ([moreMessages count] > 0) {
