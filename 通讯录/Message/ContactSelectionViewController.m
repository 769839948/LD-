...
@property (strong, nonatomic) NSMutableArray *selectedSearchContacts;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _selectedSearchContacts = [NSMutableArray array];
...
//    [self.navigationItem setLeftBarButtonItem:backItem];
    ...
    for (NSString *userName in _contactsSource) {
        [_selectedSearchContacts addObject:[[NSUserDefaults standardUserDefaults] objectForKey:userName]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.username = [[NSUserDefaults standardUserDefaults]objectForKey:username];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    [[RealtimeSearchUtil currentUtil] realtimeSearchWithSource:self.selectedSearchContacts searchText:searchText collationStringSelector:@selector(showName) resultBlock:^(NSArray *results) {
        if (results) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for (NSString *username in results) {
                    ...
                    for (NSString *result in weakSelf.contactsSource) {
                        if ([[[NSUserDefaults standardUserDefaults] objectForKey:result] isEqualToString:username]) {
                            [weakSelf.selectedContacts addObject:result];
                        }
                    }
                    
                    if ([weakSelf.selectedSearchContacts containsObject:username])
                    {
...
- (void)reloadFooterView
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([[NSUserDefaults standardUserDefaults] objectForKey:username] == nil) {
            ...
            remarkView.remark = [[NSUserDefaults standardUserDefaults] objectForKey:username];
        }
...
- (void)loadDataSource
{
    ...
    NSArray *buddyList = [[NSUserDefaults standardUserDefaults] objectForKey:@"contactPhones"];
