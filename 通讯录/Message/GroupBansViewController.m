...
@interface GroupBansViewController ()<NSLayoutManagerDelegate>
{
    BOOL _isEditing;
}
        ...
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [_group.bans count]) {
                ...
                [contactView setDeleteContact:^(NSInteger index) {
                    ...
                    [weakSelf showHudInView:weakSelf.view hint:@"删除黑名单成员"];
...
{
    __weak typeof(self) weakSelf = self;
    [self showHudInView:weakSelf.view hint:@"获取黑名单列表"];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        EMError *error = nil;
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!error) {
                ...
                NSString *errorStr = [NSString stringWithFormat:@"获取黑名单列表失败%@", error.errorDescription];
