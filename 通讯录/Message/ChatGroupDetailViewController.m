...
- (UIButton *)clearButton
{
    if (_clearButton == nil) {
        ...
        [_clearButton setTitle:@"删除聊天信息" forState:UIControlStateNormal];
...
- (UIButton *)dissolveButton
{
    if (_dissolveButton == nil) {
        ...
        [_dissolveButton setTitle:@"删除群" forState:UIControlStateNormal];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        cell.textLabel.text = @"群名称设置";
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
    else if (indexPath.row == 5)
    {
        cell.textLabel.text = @"搜索历史消息";
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
...
- (BOOL)viewController:(EMChooseViewController *)viewController didFinishSelectedSources:(NSArray *)selectedSources
{
    ...
    if (([selectedSources count] + _chatGroup.occupantsCount) > maxUsersCount) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"群人员上限" message:nil delegate:nil cancelButtonTitle:@"确定"otherButtonTitles:nil, nil];
        [alertView show];
        
        return NO;
    }
    
    [self showHudInView:self.view hint:@"添加群成员"];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *messageStr = [NSString stringWithFormat:NSLocalizedString(@"邀请你加入", @"%@ 邀请你加入 \'%@\'"), username, weakSelf.chatGroup.subject];
        EMError *error = nil;
        ...
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                [contactView setDeleteContact:^(NSInteger index) {
                    [weakSelf showHudInView:weakSelf.view hint:@"删除群成员"];
...
{
    __weak typeof(self) weakSelf = self;
    [self showHudInView:self.view hint:@"删除群"];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(){
        EMError *error = nil;
