...
{
    if (_footerView == nil) {
        _footerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width , 80)];
        ...
        _footerButton = [[UIButton alloc] initWithFrame:CGRectMake(10, 20, _footerView.frame.size.width - 20, 40)];
...
- (void)reloadSubviewsInfo
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([weakSelf isJoined:weakSelf.group]) {
            ...
            [weakSelf.footerButton setTitle:@"申请入群" forState:UIControlStateNormal | UIControlStateDisabled];
        }
        else{
            weakSelf.footerButton.enabled = YES;
            [weakSelf.footerButton setTitle:@"申请入群"  forState:UIControlStateNormal];
        }
...
{
    [self showHudInView:self.view hint:@"发送同意申请"];
